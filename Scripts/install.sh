...
mkdir -p "$HOME"/Pictures/Code
mkdir -p "$HOME"/Pictures/Screenshots
mkdir -p "$HOME"/Downloads

# Track files in the restored places
init_git() {
  local dir="$1"
  if [[ ! -d "$dir" ]]; then
  ...
  git -C "$dir" init -q
  git -C "$dir" add .
  # git -C "$dir" commit -m "track: commit" || echo "No changes to commit in $dir"
}
...
if [[ ! -d ~/.config/nvim ]]; then
  git clone https://github.com/abhidahal/avim ~/.config/nvim
fi
if [[ ! -d ~/.config/tmux/plugins/tpm ]]; then
  git clone https://github.com/tmux-plugins/tpm ~/.config/tmux/plugins/tpm
  rm -rf ~/.config/tmux/plugins/tpm/.git
fi
print_log -stat "Install dev npm packages? (y/N)"
read -r answer
if [[ "$answer" == [Yy] ]]; then
