...
if pkg_installed greetd; then
  ...
  if [ ! -d /etc/greetd/ ]; then
    [ "${flg_DryRun}" -eq 1 ] || sudo mkdir -p /etc/greetd/
  fi
  if [ ! -f /etc/greetd/config.toml ]; then
    print_log -g "[DISPLAYMANAGER] " -b " :: " "configuring greetd..."
    sudo cp "${cloneDir}/Configs/greetd/config.toml" /etc/greetd/config.toml
  fi
  if [ ! -f /etc/greetd/hyprland.conf ]; then
    print_log -g "[DISPLAYMANAGER] " -b " :: " "configuring greetd for hyprland..."
    sudo cp "${cloneDir}/Configs/greetd/hyprland.conf" /etc/greetd/hyprland.conf
  fi

  if pkg_installed greetd; then
    print_log -g "[DISPLAYMANAGER] " -b " :: " "configuring regreet"
    if [ ! -f /etc/greetd/regreet.toml ]; then
      sudo cp "${cloneDir}/Configs/greetd/regreet.toml" /etc/greetd/regreet.toml
    fi
  else
    print_log -y "[DISPLAYMANAGER] " -b " :: " "regreet is not installed..."
  fi
else
